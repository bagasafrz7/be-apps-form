...
const generateAccessToken = async (payload) => {
    return jsonwebtoken.sign({payload}, env.JWT_ACCESS_TOKEN_SECRET, {expiresIn: env.JWT_ACCESS_TOKEN_EXPIRED_TIME})
}
const generateRefreshToken = async (payload) => {
    return jsonwebtoken.sign({payload}, env.JWT_ACCESS_REFRESH_TOKEN_SECRET, {expiresIn: env.JWT_ACCESS_REFRESH_TOKEN_EXPIRED_TIME})
}
...
class AuthController {
    ...
    async login(req, res) {
        ...
        try {
            ...
            if(!password) { throw {code: 400, message: 'Invalid Password'} }

            const accessToken = await generateAccessToken({id: user._id})
            const refreshToken = await generateRefreshToken({id: user._id})

            return res.status(200).json({status: true, message: 'Login Success', accessToken, refreshToken, data: {fullname: user.fullname}})
        } catch (err) {
            return res.status(err.code || 500).json({status: false, message: err.message});
        }
    }
    ...
    async refreshToken(req, res) {
        try {
            if(!req.body.refreshToken) { throw { code: 400, message: 'Refresh token required!'}} 
    
            const verifyToken = await jsonwebtoken.verify(req.body.refreshToken, env.JWT_ACCESS_REFRESH_TOKEN_SECRET);

            let payload = { id: verifyToken.id }
            const accessToken = await generateAccessToken(payload)
            const refreshToken = await generateRefreshToken(payload)

            return res.status(200).json({status: true, message: 'Refresh Token Success', accessToken, refreshToken}) 
        } catch (err) {
